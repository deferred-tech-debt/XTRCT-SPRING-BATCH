...
public class LUIncentiveType implements Comparable<LUIncentiveType>{


    private static final IdentityGenerator IDENTITY = IdentityGenerators.createIdentityGenerator(LUIncentiveType.class);

    @DSVColumn(header = "IncentiveTypeID", qualified = false)
    private String id;
    @DSVColumn(header = "Description")
    private String description;

    public LUIncentiveType(final String id, final String description) {
        this.id = id;
    ...
    @Override
    public int compareTo(final LUIncentiveType o) {
        if (o == this) {
            return 0;
        }
