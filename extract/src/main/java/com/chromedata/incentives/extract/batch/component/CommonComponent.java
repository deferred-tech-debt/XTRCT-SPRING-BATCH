package com.chromedata.incentives.extract.batch.component;

import com.chromedata.incentives.extract.CommandLineArgs;
...
import static com.chromedata.incentives.extract.batch.component.ExtractConstants.COMPLETE_STATUS;
...
public class CommonComponent {
    ...
    public static synchronized void updateJobExecutionCompleteStatus(final JobExecution jobExecution, final int status) {
        ...
        Integer completeStatus = (Integer)executionContext.get(COMPLETE_STATUS);
        completeStatus = completeStatus == null ? -1 : completeStatus;
        if(completeStatus < status) {
            executionContext.put(COMPLETE_STATUS, status);
        }

        if(status == EXIT_STATUS_FAIL) {
            jobExecution.stop();
        }
    }
}
